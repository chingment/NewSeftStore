    ...
    public class ProductSkuService : BaseDbContext
    {
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                ...
                if (type == OperateSlotType.MachineSlotRemove)
                {
                    ...
                    if (sellChannelStock != null)
                    {
                        ...
                        if (lockQuantity > 0)
                        {
                            MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.MachineCabinetSlotSave, string.Format("货道:{0}删除失败，存在有预定数量不能删除", slotId));
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.MachineCabinetSlotSave, string.Format("删除货道：{1}，移除实际库存：{0}", sellChannelStock.SumQuantity, slotId));
                    }
                ...
                else if (type == OperateSlotType.MachineSlotSave)
                {
                    ...
                    if (sellChannelStock == null)
                    {
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.MachineCabinetSlotSave, string.Format("初次录入货道：{0}", slotId));
                    }
                    else
                    {
                        ...
                        if (sellChannelStock.PrdProductSkuId != productSkuId)
                        {
                            ...
                            if (lockQuantity > 0)
                            {
                                ...
                                MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.MachineCabinetSlotRemove, string.Format("货道：{0}，删除失败，存在有预定数量不能删除", slotId));
                            ...
                            MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.MachineCabinetSlotSave, string.Format("货道：{1}，变换货道，实际库存：{0}", sellChannelStock.SumQuantity, slotId));
                        }
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                ...
                switch (type)
                {
                    case OperateStockType.OrderReserveSuccess:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderReserveSuccess, string.Format("货道：{1},预定成功，未支付,减少可售库存：{0}，增加待支付库存：{0}，实际库存不变", quantity, slotId));
                    ...
                    case OperateStockType.OrderCancle:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderCancle, string.Format("货道：{1},未支付，取消订单，增加可售库存：{0},减少未支付库存：{0}，实际库存不变", quantity, slotId));
                    ...
                    case OperateStockType.OrderPaySuccess:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderCancle, string.Format("货道：{1},成功支付，待取货，减少待支付库存：{0}，增加待取货库存：{0}，可售库存不变，实际库存不变", quantity, slotId));
                        result = new CustomJsonResult(ResultType.Success, ResultCode.Success, "操作成功");
                    ...
                    case OperateStockType.OrderPickupOneSysMadeSignTake:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderPickupOneSysMadeSignTake, string.Format("货道：{1},成功取货，减少实际库存：{0}，减少待取货库存：{0}，可售库存不变", quantity, slotId));
                    ...
                    case OperateStockType.OrderPickupOneManMadeSignTakeByNotComplete:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderPickupOneManMadeSignTakeByNotComplete, string.Format("货道：{1},人为标记为取货成功，减去实际库存：{0}，减去待取货库存：{0}，可售库存不变", quantity, slotId));
                    ...
                    case OperateStockType.OrderPickupOneManMadeSignNotTakeByComplete:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderPickupOneManMadeSignNotTakeByComplete, string.Format("货道：{1},系统已经标识出货成功，但实际未取货成功，人为标记未取货成功，增加可售库存：{0}，增加实际库存：{0}", quantity, slotId));
                    ...
                    case OperateStockType.OrderPickupOneManMadeSignNotTakeByNotComplete:
                        ...
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.OrderPickupOneManMadeSignNotTakeByNotComplete, string.Format("货道：{1},人为标记未取货成功，恢复可售库存：{0},减去待取货库存：{0},实际库存不变", quantity, slotId));
        ...
        public CustomJsonResult AdjustStockQuantity(string operater, string merchId, string storeId, string machineId, string cabinetId, string slotId, string productSkuId, int version, int sumQuantity, int? maxQuantity = null)
        {
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                ...
                if (sellChannelStock == null)
                {
                    MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.AdjustStockQuantity, string.Format("机柜：{0},货道：{1},保存失败，找不到该数据", cabinetId, slotId));
                ...
                if (sellChannelStock.Version != -1)
                {
                    ...
                    {
                        MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.AdjustStockQuantity, string.Format("机柜：{0},货道：{1},保存失败，数据已经被更改", cabinetId, slotId));
                ...
                MqFactory.Global.PushEventNotify(operater, AppId.STORETERM, merchId, storeId, machineId, EventCode.AdjustStockQuantity, string.Format("库存调整数量为：{0}", sumQuantity));
