                        ...
                        switch (m.Type)
                        {
                            ...
                            case Task4TimType.PayRefundCheckStatus:
                                ...
                                if (m.ExpireTime.AddMinutes(1) >= DateTime.Now)
                                {
                                    ...
                                    MqFactory.Global.PushPayRefundResultNotify(IdWorker.Build(IdType.EmptyGuid), payRefund.PayPartner, payRefund.Id, payRefund.PayTransId, E_PayTransLogNotifyFrom.Query, content);
                                }
                                else
                                {
