    ...
    public class OwnService : BaseDbContext
    {
        ...
        public CustomJsonResult LoginByAccount(RopOwnLoginByAccount rop)
        {
            ...
            if (rop.AppId == AppId.STORETERM)
            {
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), "", EventCode.Login, "登录失败，缺少参数loginPms", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "缺少参数loginPms" });
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), "", EventCode.Login, "登录失败，缺少参数loginPms.machineId", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "缺少参数loginPms.machineId" });
            ...
            if (sysUser == null)
            {
                MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，账号不存在", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "账号不存在" });
            ...
            {
                MqFactory.Global.PushOperateLog(rop.AppId, sysUser.Id, machineId, EventCode.Login, "登录失败，密码不正确", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "密码不正确" });
                return new CustomJsonResult(ResultType.Failure, ResultCode.Failure, "登录失败，密码不正确");
            }

            if (sysUser.IsDisable)
            {
                MqFactory.Global.PushOperateLog(rop.AppId, sysUser.Id, machineId, EventCode.Login, "登录失败，账号已被禁用", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表SysUser字段IsDisable:True" });
            ...
            else if (rop.AppId == AppId.STORETERM)
            {
                ...
                if (machine == null)
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, sysUser.Id, machineId, EventCode.Login, "登录失败，该机器未登记", new LoginLogModel { LoginAccount = rop.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = string.Format("表Machine字段Id:0}，找不到", machineId) });
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "录失败，该机器未绑定商家", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表Machine字段CurUseMerchId，为空" });
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "录失败，该机器未绑定店铺", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表Machine字段CurUseStoreId,为空" });
            ...
            MqFactory.Global.PushOperateLog(rop.AppId, sysUser.Id, machineId, EventCode.Login, "登录成功", new LoginLogModel { LoginAccount = sysUser.UserName, LoginFun = Enumeration.LoginFun.Account, LoginResult = Enumeration.LoginResult.LoginSuccess, LoginWay = rop.LoginWay, LoginIp = rop.Ip });
        ...
        public CustomJsonResult LoginByMinProgram(RopOwnLoginByMinProgram rop)
        {
            ...
            MqFactory.Global.PushOperateLog(rop.AppId, wxUserInfo.Id, "", EventCode.Login, "登录成功");
        ...
        public CustomJsonResult LoginByFingerVein(RopOwnLoginByFingerVein rop)
        {
            ...
            if (rop.AppId == AppId.STORETERM)
            {
                if (rop.LoginPms == null)
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，缺少参数loginPms", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "登录失败，缺少参数loginPms" });
                    return new CustomJsonResult(ResultType.Failure, ResultCode.Failure, "登录失败，缺少参数loginPms");
                }

                if (!rop.LoginPms.ContainsKey("machineId") || string.IsNullOrEmpty(rop.LoginPms["machineId"].ToString()))
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，缺少参数loginPms.machineId", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "登录失败，缺少参数loginPms.machineId" });
                ...
                if (machine == null)
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，该机器未登记", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = string.Format("表Machine字段Id:{0}，找不到", machineId) });
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，该机器未绑定商家", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表Machine字段CurUseMerchId，为空" });
                ...
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，该机器未绑定店铺", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表Machine字段CurUseStoreId，为空" });
            ...
            {
                MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "暂不支持", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "暂不支持" });
            ...
            if (!isMachSuccess)
            {
                MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "静指脉验证失败", new LoginLogModel { LoginAccount = "", LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "静指脉验证失败" });
            ...
            if (isMachSuccess)
            {
                ...
                if (sysUser == null)
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, GuidUtil.Empty(), machineId, EventCode.Login, "登录失败，账号不存在", new LoginLogModel { LoginAccount = sysUser.UserName, LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "账号不存在" });
                ...
                if (sysUser.IsDisable)
                {
                    MqFactory.Global.PushOperateLog(rop.AppId, sysUser.Id, machineId, EventCode.Login, "登录失败，账号已被禁用", new LoginLogModel { LoginAccount = sysUser.UserName, LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginFailure, LoginWay = rop.LoginWay, RemarkByDev = "表SysUser字段IsDisable:True" });
                    return new CustomJsonResult(ResultType.Failure, ResultCode.Failure, "登录失败，账号已被禁用");
                }
                ...
                MqFactory.Global.PushOperateLog(rop.AppId, userId, machineId, EventCode.Login, "登录成功", new LoginLogModel { LoginAccount = sysUser.UserName, LoginFun = Enumeration.LoginFun.FingerVein, LoginResult = Enumeration.LoginResult.LoginSuccess, LoginWay = rop.LoginWay });
        ...
        public CustomJsonResult Logout(string operater, string userId, RopOwnLogout rop)
        {
            ...
            MqFactory.Global.PushOperateLog(rop.AppId, operater, "", EventCode.Logout, "退出成功", new LoginLogModel { LoginResult = Enumeration.LoginResult.LogoutSuccess, LoginWay = rop.LoginWay });
