...
namespace LocalS.Service.Api.StoreApp
{
    public class CartService : BaseDbContext
    {
        public CustomJsonResult<RetCartPageData> PageData(string operater, string clientUserId, RupCartPageData rup)
        {
            ...
            {
                var bizProductSku = CacheServiceFactory.ProductSku.GetInfoAndStock(clientCart.MerchId, store.Id, store.MachineIds, clientCart.PrdProductSkuId);
                    ...
                    foreach (var item in rop.ProductSkus)
                    {
                        ...
                        switch (rop.Operate)
                        {
                            ...
                            case E_CartOperateType.Increase:
                                ...
                                var bizProductSku = CacheServiceFactory.ProductSku.GetInfoAndStock(store.MerchId, store.Id, store.MachineIds, item.Id);
