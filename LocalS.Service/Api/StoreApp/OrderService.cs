    ...
    public class OrderService : BaseDbContext
    {
        ...
        public CustomJsonResult Confrim(string operater, string clientUserId, RopOrderConfirm rop)
        {
            ...
            else
            {
                ...
                ret.CouponByDeposit = new OrderConfirmCouponModel { TipMsg = amount_couponByShop == 0 ? "无优惠" : string.Format("-{0}", amount_couponByDeposit.ToF2Price()), TipType = TipType.InUse };
                ret.CouponByRent = new OrderConfirmCouponModel { TipMsg = amount_couponByShop == 0 ? "无优惠" : string.Format("-{0}", amount_couponByRent.ToF2Price()), TipType = TipType.InUse };
