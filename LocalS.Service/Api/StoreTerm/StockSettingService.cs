...
namespace LocalS.Service.Api.StoreTerm
{
    public class StockSettingService : BaseDbContext
    {
        public CustomJsonResult GetCabinetSlots(string operater, RupStockSettingGetCabinetSlots rup)
        {
            ...
            MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, machine.MerchId, machine.StoreId, machine.Id, EventCode.MachineCabinetGetSlots, "查看库存");
        ...
        public CustomJsonResult SaveCabinetSlot(string operater, RopStockSettingSaveCabinetSlot rop)
        {
            ...
            if (string.IsNullOrEmpty(rop.ProductSkuId))
            {
                ...
                {
                    MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, machine.MerchId, machine.StoreId, rop.MachineId, EventCode.MachineCabinetSlotSave, "移除货道商品成功");
                }
                else
                {
                    MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, machine.MerchId, machine.StoreId, rop.MachineId, EventCode.MachineCabinetSlotSave, "移除货道商品失败");
                }
            ...
            else
            {
                ...
                if (result.Result == ResultType.Success)
                {
                    ...
                    if (result.Result == ResultType.Success)
                    {
                        MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, machine.MerchId, machine.StoreId, rop.MachineId, EventCode.MachineCabinetSlotSave, "保存货道商品成功");
                    }
                    else
                    {
                        MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, machine.MerchId, machine.StoreId, rop.MachineId, EventCode.MachineCabinetSlotSave, "保存货道商品失败");
                    }
                }
                else
                {
        ...
        public CustomJsonResult SaveCabinetRowColLayout(string operater, RopStockSettingSaveCabinetRowColLayout rop)
        {
            ...
            if (result.Result == ResultType.Success)
            {
                MqFactory.Global.PushOperateLog(operater, AppId.STORETERM,"","",  rop.MachineId, EventCode.MachineSaveCabinetRowColLayout, "保存柜子货道扫描结果成功");
            }
            else
            {
                MqFactory.Global.PushOperateLog(operater, AppId.STORETERM, "", "" ,rop.MachineId, EventCode.MachineSaveCabinetRowColLayout, "保存柜子货道扫描结果失败");
            }
