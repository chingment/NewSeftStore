...
namespace Test
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第一门店", DeviceCode = "A1", Quantity = 1, SkuName = "P1" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第一门店", DeviceCode = "A3", Quantity = 1, SkuName = "P1" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第一门店", DeviceCode = "A4", Quantity = 1, SkuName = "P2" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第二门店", DeviceCode = "A7", Quantity = 1, SkuName = "P2" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第二门店", DeviceCode = "A6", Quantity = 1, SkuName = "P2" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第三门店", DeviceCode = "A2", Quantity = 2, SkuName = "P1" });
            stocks.Add(new Stock { StoreName = "贩聚社团", ShopName = "第三门店", DeviceCode = "A5", Quantity = 3, SkuName = "P3" });



            var ds1 = (from s in stocks
                       group s by new { s.StoreName } into g
                       select new { g.Key, Total = g.Count() }).ToList();

            var a1 = "";

            for (var i = 0; i < ds1.Count; i++)
            {
                var html1 = "<tr>";


                html1 += "<td rowspan=\"" + ds1[i].Total + "\" >" + ds1[i].Key.StoreName + "</td>";


                var ds2 = (from s in stocks
                           group s by new { s.SkuName } into g
                           select new { g.Key, Total = g.Count() }).ToList();
                for (var j = 0; j < ds2.Count; j++)
                {
                    var html2 = "<tr>";

                    if (j == 0)
                    {
                        html1 += "<td  rowspan=\"" + ds2[j].Total + "\">" + ds2[j].Key.SkuName + "</td>";
                    }
                    else
                    {
                        html2 += "<td  rowspan=\"" + ds2[j].Total + "\">" + ds2[j].Key.SkuName + "</td>";
                    }

                    var skuName = ds2[j].Key.SkuName;
                    var ds3 = (from s in stocks
                               where s.SkuName == skuName
                               group s by new { s.ShopName } into g
                               select new { g.Key, Total = g.Count() }).ToList();



                    for (var x = 0; x < ds3.Count; x++)
                    {


                        var shopName = ds3[x].Key.ShopName;

                        var ds4 = (from s in stocks
                                   where s.SkuName == skuName
                                   && s.ShopName == shopName
                                   group s by new { s.DeviceCode, s.SkuName, s.Quantity } into g
                                   select new { g.Key, Total = g.Count() }).ToList();
                        ...
                        if (i == 0 && j == 0 && x == 0)
                        {
                            html1 += "<td rowspan=\"" + ds4.Count + "\">" + ds3[x].Key.ShopName + "</td>";
                        }
                        else
                        {
                            if (j == 0)
                            {
                                html3 += "<td>" + ds3[x].Key.ShopName + "</td>";
                            }
                            else {
                                html2 += "<td rowspan=\"" + ds4.Count + "\">" + ds3[x].Key.ShopName + "</td>";
                            }
                        }

                        for (var s = 0; s < ds4.Count; s++)
                        {


                            if (i == 0 && j == 0 && x == 0 && s == 0)
                            {
                                html1 += "<td>" + ds4[s].Key.DeviceCode + "</td>";
                                html1 += "<td>" + ds4[s].Key.Quantity + "</td>";
                                html1 += "</tr>";
                                a1 += html1;
                            }
                            else if (j > 0)
                            {
                                html2 += "<td>" + ds4[s].Key.DeviceCode + "</td>";
                                html2 += "<td>" + ds4[s].Key.Quantity + "</td>";
                                html2 += "</tr>";
                                a1 += html2;
                            }
                            else if (i == 0 && j == 0)
                            {
                                ...
                                {
                                    var html4 = "<tr>";
                                    html4 += "<td>" + ds4[s].Key.DeviceCode + "</td>";
                                    html4 += "<td>" + ds4[s].Key.Quantity + "</td>";
                                    html4 += "</tr>";
                                    a1 += html4;
                                }
                                else
                                {
                                    html3 += "<td>" + ds4[s].Key.DeviceCode + "</td>";
                                    html3 += "<td>" + ds4[s].Key.Quantity + "</td>";
                                    html3 += "</tr>";
                                    a1 += html3;
                                }
                            }
