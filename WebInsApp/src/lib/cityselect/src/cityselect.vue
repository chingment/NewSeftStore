<template>
  <transition name="slide">
    <div class="xin-widget-citys animated" v-if="isShow">
      ...
      <div
        class="xin-widget-citys-letnav"
        v-if="isShow && input == '' && !simple"
        @touchmove="_touchLetters"
      >
        <ol>
          <template v-for="(item, index) in letterList">
            <li v-if="isNaN(index) && index=='star'" @click="_chooseLetter" data-type="starCity">
              <em class="star-small" data-type="starCity"></em>
            </li>
        ...
        case "touchmove":
          if (touchY < olBot && touchY > olTop) {
            let olTouchY = touchY - olTop,
              ...
              target = document.querySelectorAll(".xin-widget-citys-letnav li")[
                targetIndex - 1
              ];
      ...
      if (_dtList) {
        _dtList.forEach((value, index, array) => {
          if (value.innerText === _this.targetLetter) {
            ...
            let scrollTop =
              document.querySelector(".xin-widget-citys-list").offsetTop +
              value.offsetTop;
    ...
    _search: function() {
      ...
      for (var i in this.letterList) {
        for (var j = 0; j < this.letterList[i].length; j++) {
          if (
            reg.test(this.letterList[i][j]["cityName"]) ||
            reg.test(this.letterList[i][j]["cityFirstLetter"]) ||
            reg.test(this.letterList[i][j]["citySpell"])
          ) {
