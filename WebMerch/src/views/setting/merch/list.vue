<template>
  <div class="merch_list">
    <div class="filter-container">
      <el-row :gutter="12">
        <el-col :xs="24" :sm="12" :lg="6" :xl="4" style="margin-bottom:20px">
          <el-input v-model="listQuery.name" style="width: 100%" placeholder="用户名" clearable class="filter-item" />
        </el-col>
        <el-col :xs="24" :sm="12" :lg="6" :xl="4" style="margin-bottom:20px">
    ...
    >
      ...
      <el-table-column label="商户名" prop="fullName" align="left" min-width="20%">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    onAdd() {
      this.$router.push({
        path: '/setting/merch/add'
      })
    },
    onEdit(row) {
      this.$router.push({
        path: '/setting/merch/edit?id=' + row.id
      })
    }
  }
}
